...
### 3.Explica qué es un objeto en C++ desde la perspectiva de memoria. ¿Dónde se almacenan los miembros de instancia y dónde los miembros estáticos?
Desde la perspectiva de la memoria, un objeto en C++ no es más que un bloque contiguo de bytes que representa una instancia de una clase, donde el tamaño total depende de la suma de sus variables de instancia y el relleno por alineación. Los miembros de instancia se almacenan directamente dentro de la dirección de memoria asignada al objeto, lo que significa que si el objeto se crea localmente vive en el stack, pero si se crea con el operador new sus miembros residen en el heap. Por el contrario, los miembros estáticos no forman parte del cuerpo físico del objeto y no ocupan espacio en su instancia, sino que se almacenan en el segmento de datos de la memoria de manera global. Esto implica que solo existe una copia de los miembros estáticos compartida por todas las instancias de la clase, independientemente de si los objetos fueron creados en el stack o en el heap. 
